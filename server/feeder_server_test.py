...
class Feeder_server:
    ...
    def state_server_thread(self):
        while self.r_state_socks:
            #print('state test')
            readEvent, writeEvent, errorEvent = select.select(self.r_state_socks, [], self.r_state_socks)
            ...
            for s in readEvent:                                     # 읽기 가능 소켓 조사
                if s is self.cmd_server_socket:                     # 서버 소켓?
                    ...
                    try:
                        ...
                        if "ID" in data:
                            print('test ID')
                            self.feeder_list[data["ID"]] = s        # {'F-01':socket}
    ...
    def get_feeder_list(self):
        #self.info정보를 통해 현재 연결된 급이기 아이디 리스트로 출력
